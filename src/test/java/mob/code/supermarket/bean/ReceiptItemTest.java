...
public class ReceiptItemTest {
    ...
    public void 当类型是0数量不是整数抛出异常() throws Exception {
        ReceiptItem receiptItem = new ReceiptItem("", 2.1, 2.0, "", "0", new BuyItem("123456", 2.1, ""));
    }
    @Test(expected = WrongQuantityException.class)
    public void 当数量是0抛出异常() throws Exception {
        ReceiptItem receiptItem = new ReceiptItem("", 0, 2.0, "", "0", new BuyItem("123456", 0, ""));
    }
    @Test
    public void 当类型是0数量是整数_不抛出异常() throws Exception {
        ReceiptItem receiptItem = new ReceiptItem("", 2.0, 2.0, "", "0", new BuyItem("123456", 2.0, ""));
    }

    @Test(expected = WrongQuantityException.class)
    public void Given类型是1_When数量是两位小数_Then抛出异常() throws Exception {
        ReceiptItem receiptItem = new ReceiptItem("", 2.11, 2.0, "", "1", new BuyItem("123456", 2.11, ""));
    }
    @Test(expected = WrongQuantityException.class)
