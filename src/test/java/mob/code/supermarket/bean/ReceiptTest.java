...
public class ReceiptTest {
    ...
    public void 当有一个商品() throws Exception {
        Receipt receipt = Receipt.of(Arrays.asList(new ReceiptItem("pizza", 1, 15.00, "", type, new BuyItem("", 1, "1123-1"))));
    ...
    public void 当有一个商品带有单位() throws Exception {
        Receipt receipt = Receipt.of(Arrays.asList(new ReceiptItem("milk", 1, 15.00, "L", type, new BuyItem("", 1, "1123-1"))));
    ...
    public void 当有1个商品有2份() throws Exception {
        Receipt receipt = Receipt.of(Arrays.asList(new ReceiptItem("milk", 2, 15.00, "L", type, new BuyItem("", 2, "1123-1"))));
    ...
    public void 当类型是1的时候数量保留1位() throws Exception {
        Receipt receipt = Receipt.of(Arrays.asList(new ReceiptItem("milk", 2.2, 15.00, "L", "1", new BuyItem("", 2.2, "1123-1"))));
        assertThat(receipt.output().get(1), is("milk: 2.2(L) x 15.00 --- 33.00"));
    }

    @Test
    public void 当类型是1的时候数量小数超过2保留1位() throws Exception {
        Receipt receipt = Receipt.of(Arrays.asList(new ReceiptItem("milk", 2.2000000001, 15.00, "L", "1", new BuyItem("", 2.20000001, "1123-1"))));
        assertThat(receipt.output().get(1), is("milk: 2.2(L) x 15.00 --- 33.00"));
    }

    @Test
    public void 当总价有尾部舍弃2位小数后面的() throws Exception {
        Receipt receipt = Receipt.of(Arrays.asList(new ReceiptItem("milk", 3.5, 15.09, "L", "1", new BuyItem("", 3.5, "1123-1"))));
    ...
    public void 当两个总价都舍弃尾部按舍弃后都累计() throws Exception {
        Receipt receipt = Receipt.of(Arrays.asList(
                new ReceiptItem("milk", 3.5, 15.09, "L", "1", new BuyItem("", 3.5, "1123-1")),
                new ReceiptItem("milk", 3.5, 15.09, "L", "1", new BuyItem("", 3.5, "1123-1"))
        ));
